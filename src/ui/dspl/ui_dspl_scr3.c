...
void    ui_dspl_scr3_cb(                        WM_MESSAGE *            pMsg )
{
        ...
        //APP_TRACE_EMWIN( "ui_dspl_scr3_cb()\t", pMsg->MsgId );
...
*/
                case WM_NOTIFY_PARENT:
                        ...
                        switch( NCode )
                        {
                                ...
                                //case WM_NOTIFICATION_RELEASED:                  // React only if released
                                        if( Id == GUI_ID_SCR3_BUTTON_STRT )
                                        {
                                                BSP_LED_Toggle( LED1 );
                                                hItem   =   WM_GetDialogItem( hWin, GUI_ID_SCR3_TEXT_TEMP );
                                                TEXT_SetText( hItem, "*** ÑÒÀÐÒ ***" );
                                                //WM_Invalidate( hWin );
                                        }

                                        if( Id == GUI_ID_SCR3_BUTTON_CLR )
                                        {
                                                BSP_LED_Toggle( LED2 );
                                                hItem   =   WM_GetDialogItem( hWin, GUI_ID_SCR3_TEXT_TEMP );
                ...
                case WM_INIT_DIALOG:
                        ...
                        //hItem   =   WM_GetDialogItem( pMsg->hWin, GUI_ID_SCR3_GRAPH );
                        //ui_dspl_scr0_add_graph( hItem, GUI_ID_SCR0_GRAPH );

                        hItem   =   WM_GetDialogItem( hWin, GUI_ID_SCR3_BUTTON_STRT );
                        //BUTTON_SetFocusColor( hItem, GUI_YELLOW );
                        BUTTON_SetFocusable( hItem, 0 );

                        hItem   =   WM_GetDialogItem( hWin, GUI_ID_SCR3_BUTTON_CLR );
                        //BUTTON_SetFocusColor( hItem, GUI_YELLOW );
                        BUTTON_SetFocusable( hItem, 0 );
